...
(function () {
    ...
    // English defaults are stored in the HTML itself

    var currentLang = localStorage.getItem('lang') || 'en';
    var originalTexts = {};

    // Store original English text from DOM on first load
    ...
    function applyLanguage(lang) {
        var isZh = lang === 'zh-HK';
        var dict = isZh ? translations['zh-HK'] : null;

        // Update html lang
        document.documentElement.lang = isZh ? 'zh-HK' : 'en';

        // Update body class for CJK font
        document.body.classList.toggle('lang-zh', isZh);

        // Update page title & meta
        if (isZh) {
            document.title = 'Kaho Tsang — 網頁、應用程式及自動化顧問';
            var metaDesc = document.querySelector('meta[name="description"]');
            if (metaDesc) metaDesc.setAttribute('content', '我幫助企業建立更好的數碼產品，並改善營運流程。網頁設計、應用程式開發、工作流程自動化及流程改善。');
        } else {
            document.title = 'Kaho Tsang — Web, App & Automation Consultant';
            var metaDescEn = document.querySelector('meta[name="description"]');
            if (metaDescEn) metaDescEn.setAttribute('content', 'I help businesses build better digital products and improve the way they work. Web design, app development, workflow automation, and process improvement in Hong Kong.');
        }
        ...
        // Translate HTML blocks (What I Do, Why lead)
        document.querySelectorAll('[data-i18n-html]').forEach(function (el) {
        ...
        if (labelMobileEl) labelMobileEl.textContent = toggleLabel;
